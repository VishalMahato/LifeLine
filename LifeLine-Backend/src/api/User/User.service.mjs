...
export default class UserService {
  ...
  static async getUserById(userId) {
    try {
      const user = await User.findById(userId).populate('medicalId').populate('locationId');
  ...
  static async getUserByAuthId(authId) {
    try {
      const user = await User.findOne({ authId }).populate('medicalId').populate('locationId');
  ...
  static async updateUser(userId, updateData) {
    try {
      ...
      if (
        ...
        !UserUtils.validateEmergencyContacts(updateData.emergencyContacts)
      ) {
        throw new Error(UserConstants.MESSAGES.VALIDATION.INVALID_EMERGENCY_CONTACTS);
      }
