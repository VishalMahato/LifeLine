  ...
  static async verifyCredentials(helperId, credentialId, status) {
    try {
      ...
      credential.verifiedAt =
        status === HelperConstants.VERIFICATION_STATUS.VERIFIED ? new Date() : null;

      // Update overall verification status
      const allVerified = helper.credentials.every(
        (cred) => cred.verificationStatus === HelperConstants.VERIFICATION_STATUS.VERIFIED,
      );
  ...
  static async searchHelpers(filters = {}, options = {}) {
    try {
      ...
      const [helpers, total] = await Promise.all([
        Helper.find(query).sort(sortOptions).skip(skip).limit(limit).populate('locationId'),
